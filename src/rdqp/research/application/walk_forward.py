...
class WalkForwardEngine:
    ...
    def run(
        ...
        for fold in range(folds):
            train_end_index = min(train_size + fold * test_size, len(timestamps) - 1)
            test_end_index = min(train_end_index + test_size, len(timestamps) - 1)
            if train_end_index >= test_end_index:
                break
            train_start, train_end = timestamps[0], timestamps[train_end_index]
            test_start, test_end = timestamps[train_end_index + 1], timestamps[test_end_index]
        ...
        return WalkForwardResult(tuple(fold_results), combined - 1.0 if returns else 0.0, average)

    @staticmethod
    def _slice(
        histories: Mapping[str, Sequence[FactorSnapshot]], start: datetime, end: datetime
