...
def snap(symbol="AAPL", roc=.02, rank=1):
    return FactorSnapshot(symbol, datetime.now(timezone.utc), 100.0, 1000, "Technology", roc, 2.0, 99.0, .01, .0, 101, 98, "inside", rank)


def strategy():
    return StrategyDefinition("auto", (StrategyRule("rank", RuleOperator.LTE, 2),), (StrategyRule("roc_pct", RuleOperator.LT, 0),))
...
def test_dry_run_does_not_submit(tmp_path):
    ...
    run = runner.run_cycle(strategy(), [snap()], AutomationConfig(mode=AutomationMode.DRY_RUN), RiskLimits())
...
def test_paper_armed_submits(tmp_path):
    runner = AutomationRunner(manager(tmp_path))
    run = runner.run_cycle(strategy(), [snap()], AutomationConfig(mode=AutomationMode.PAPER_ARMED), RiskLimits())
...
def test_jsonl_journal(tmp_path):
    ...
    runner.run_cycle(strategy(), [snap()], AutomationConfig(mode=AutomationMode.DRY_RUN), RiskLimits())
